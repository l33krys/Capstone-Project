...
Each data set was separated into a training and testing set. By creating two separate sets we can use the training set to create a prediction model and then add in our testing set to see how well it performs. This is a good way to see if we have overfitted our data. Our goal is to find a model that performs similar or better with the testing set.
...
Winemakers can use these prediction models created to see if the wines they're producing are good quality wines. By having this understanding of their wines, it will empower winemakers to more informed decisions on what to do with their wines. If results show they have produced bad quality wines, they can repurpose them to create wine blends or eliminate them altogether to reduce any further costs. If the wines are good quality, they can have relief in knowing that they're providing their customers with a great wine experience. 
